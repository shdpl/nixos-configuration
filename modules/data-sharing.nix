...
let
  ...
  syncthing = (import ../private/syncthing/net.nawia.nix);
in
{
  # imports = [
  ...
  config = (mkMerge [
    ...
    #   };
    # })
    (mkIf (config.dataSharing.enable == true) {
      services.syncthing = {
        enable = true;
        user = user;
        ...
        settings = {
          ...
          urAccepted = 1;
          folders = (
            builtins.head (builtins.filter (device: device.name == host) syncthing.devices)
          ).folders;
          devices = builtins.listToAttrs (
            builtins.map
              (device: {name=device.name; value={id=device.id; addresses=device.addresses;};})
              (builtins.filter (device: device.name != host) syncthing.devices)
          );
        };
      };
    })
    # (mkIf (config.dataSharing.enable == true && config.dataSharing.vhost != "") {
