...
let
	cfg = config.mateuszmickiewiczPl;
	credentials = import ../../private/website/mateuszmickiewicz_pl.nix;
in
{
	options.mateuszmickiewiczPl = {
		vhost = mkOption {
			type = types.str;
		};
		dbName = mkOption {
  ...
  config = (mkMerge [
        ...
        '';
			...
			systemd = {
				timers.mateuszmickiewiczPlBackup = {
					wantedBy = [ "timers.target" ];
					timerConfig = {
						OnCalendar = "01:15:00";
						AccuracySec = "5m";
						Unit = "mateuszmickiewiczPlBackup.service";
					};
				};
				services."mateuszmickiewiczBackup" = {
					enable = true;
					serviceConfig = {
						User = cfg.backupUser;
